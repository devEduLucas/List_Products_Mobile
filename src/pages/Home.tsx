...
export default function Home() {
    ...
    function quantidade(){
        if(!quantity.toString){
            console.log(`Quantidade aceita apenas números`)
        }
    }

    const generateId = (products: Products[]) => {
        const nextId = products.length === 0 ? 1 : Math.max(...products.map( products => Number(products.id))) + 1
    ...
    const handleAddTask = async () => {
        if(!newTitle.trim()){
            Alert.alert("Atenção, Digite uma Descrição para o produto!");
            return
        }
        ...
        await saveProducts([newProducts, ...products])
        setNewTitle("");
    }
    ...
    const handleSaveEdit = async () => {
        ...
        if(!editTitle.trim()){
            Alert.alert(`Atenção, o titulo não pode ser vazio!`)
        }

        const updateList = products.map((product) => product.id === editingProduct.id ?
    {... product, title: editTitle.trim()} : product)

    setProducts(updateList);
    setEditingProduct(null);
    setEditTitle('');
    await saveProducts(updateList);
    }
    ...
    return(
        <SafeAreaView>
            ...
            <View style={styles.InputContainer}>
                <TextInput style={styles.Input} placeholder="Digite o nome do produto..." placeholderTextColor='#94A3B8' value={newTitle} onChangeText={setNewTitle} onSubmitEditing={handleAddTask} returnKeyType="done">
                </TextInput>
	...
	<View style={styles.textContainer}>
        ...
        <TextInput value={quantity} onChangeText={setQuantity} placeholder="Add quantidade">
        </TextInput>
